...
```{r table-functions-management, echo = FALSE}
out <- tribble(
  ~"Function", ~"What it does ...", ~"Lifecycle stage", ~"Chapter",
  #
  "scdf", "Creates a single-case data-frame", "Stable", "\\@ref(scdf)",
  "select_cases", "Selects specific cases of an scdf", "Stable", "\\@ref(select-cases)",
  "select_phases", "Selects and/or recombines phases", "Stable", "\\@ref(select-phases)",
  "subset", "Selects specific measurements or variables of an scdf", "Stable", "\\@ref(subset)",
  "read_scdf", "Loads external data into an scdf", "Stable", "\\@ref(read-scdf)",
  "write_scdf", "Writes scdf into an external file", "Stable", "\\@ref(write-scdf)",
  "convert", "Converts an scdf object into R syntax", "Stable", "\\@ref(convert)",
  ...
  "add_l2", "Adds level-two data to an scdf", "Stable", "\\@ref(add-l2)",
  "as.data.frame/as.data.frame.scdf", "Transforms an scdf into a data frame", "Stable", "-"
)

print_table(out, "Functions for data management.",)

```

### Depiction

```{r table-functions-depiction, echo = FALSE}
out <- tribble(
...
print_table(out, "Functions for data depiction/visualisation.",)

```

### Analysis

```{r table-functions-analysis, echo = FALSE}
out <- tribble(
  ~"Function", ~"What it does ...", ~"Lifecycle stage",
  #
...
print_table(out, "Functions for data analysis.",)

```

### Manipulation

```{r table-functions-manipulation, echo = FALSE}
out <- tribble(
  ~"Function", ~"What it does ...", ~"Lifecycle stage",
  #
...
print_table(out, "Functions for data manipulation.",)

```

### Simulation

```{r table-functions-simulation, echo = FALSE}
out <- tribble(
  ~"Function", ~"What it does ...", ~"Lifecycle stage",
  #
...
print_table(out, "Functions for data simulation.",)
