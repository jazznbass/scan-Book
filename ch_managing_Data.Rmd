...
## Convert an scdf object back to scan syntax {#convert}
...
# summary(new_huber)
```
### Select measurements {#subset}

The `subset()` function helps with extracting measurements (or rows) by a specific criteria from a scdf.

Subset takes a scdf as its first argument and a logical expression as the second argument (`filter`). Only measurements for which the logical argument is evaluated to be TRUE are inlcuded in the returning scdf object.
