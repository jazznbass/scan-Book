...
The `B.start` argument is only applicable when the single-case has one A-phase followed by one B-phase. It is a relict from the time when `scan` could only sign-case designs with two phases. The number assigned to `B.start` indicates the measurement-time as defined in the `mt` argument. That is, when the vector for the measurement times is `mt = c(1,3,7,10,15,17,18,20)` and `B.start = 15` the first measurement of the B-phase will start with the fifth measurement at which _mt_ = 15.  
...
Table \@ref(tab:scdf-table) shows a complete list of arguments that could be passed to the function.

```{r scdf-table, echo = FALSE}
ARG <- c("B.start","phase.design","name","dvar","pvar","mvar", "...")
WHAT <- c("The first measurement of phase B (simple coding if design is strictly AB).","A vector defining the length and label of each phase.","A name for the case.","The name of the dependent variable. By default this is 'values'.","The name of the variable containing the phase information. By default this is 'phase'.","The name of the variable with the measurement-time. The default is 'mt'.", "Any number of variables with a vector asigned to them.")

data.frame(Argument = ARG, "What it does ..." = WHAT, check.names = FALSE) %>%
  print_table("Arguments of the scdf function")
  #kable(caption = "Arguments of the scdf function", booktabs = TRUE) 
```
...
```{r}
dat <- readSC.excel("example2.xlsx",cvar = "name", pvar = "intervention", dvar = "wellbeing", mvar = "time", phase.names = c("A","B"))
summary(dat)
```
