...
We use the `scan` package to code the data. Each case consists of 25 measurements. We have three phases: pre intervention (A), during the intervention (B), and follow-up (C). Phases A and B have variable length (6, 7, 9). The cases are given three fictional names. And are combined into a single object called `study`.  
...
Now we visualize the casese:

```{r fig.width=4}
plot(study)
```

and we tweak the figure a bit:
...
Now we lets have a table of the data:

```{r message=FALSE, warning=FALSE}

export(study, kable_options = list(booktab = TRUE))

```

And we need descriptive statistics:

```{r message=FALSE, warning=FALSE}
res <- describeSC(study)
export(res, flip = TRUE, kable_options = list(booktab = TRUE))
```

Single-case data are oftentimes analyzed with overlap indeces. let us get an overview comparing phases A and B:
...
```{r message=FALSE, warning=FALSE}
res <- overlapSC(study, phases = c("A", "C"))
export(res, flip = TRUE, kable_options = list(booktab = TRUE))
```

Finally, we conduct regression analyses for each cases with a piecwise regression model:

```{r message=FALSE, warning=FALSE}
res <- plm(study$Dustin)
