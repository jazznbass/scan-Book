...
out <- tribble(
  ...
  "phase_design", "A vector defining the length and label of each phase.",
...
![How to format a single-case file in a spreadsheet program for importing into scan](images/readSC.jpg){width="300px" #fig-spreadsheet}
...
## Selecting cases and measurements

### Subsetting cases with base R syntax
...
Casenames can also be defined within a specific interval by the colon operator:
...
The `subset()` function helps with extracting measurements (or rows) by a specific criteria from an scdf.

Subset takes an scdf as its first argument and a logical expression as the second argument (`filter`). Only measurements for which the logical argument is evaluated to be TRUE are included in the returning *scdf* object.
...
With the help of the `transform()` function you can add new variables or change existing variables to each case of an an *scdf*. This might be helpful when you

- like to Z-standardise a variable,
- calculate a new variable as the sum of two existing variables,
- turn a frequency into percentages,

or in many other cases.

::: callout-note
The transform function is a method for the generic `transform` function. To call the help file you have to add the class to the functionname: `?transform.scdf` 
:::

Here is an example where the dependent variable `values` is z-standardized:

```{r}
exampleAB_z <- transform(exampleAB, values = (values - mean(values)) / sd(values) )
names(exampleAB_z) <- paste0(names(exampleAB), "_z")
smd(c(exampleAB, exampleAB_z))
