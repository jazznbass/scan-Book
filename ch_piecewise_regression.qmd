...
Level question: *Where would the values be, if the slope of phase A (slope A = trend) continued until the beginning of phase C and what is the difference to the actual value at the first measurement of phase C?*
...
#| fig-cap: LevelC and SlopeC effects.
exampleABC$Marie %>% 
  subset(phase != "B") %>%
  scplot() %>% 
  add_statline("trend", color = "darkred") %>%
...
The `levelC`-effect is the difference between the estimated first measurement of phase C (intercept B) and the extrapolated values of the trend line (slope A) at the last measurement of phase B.
...
```
$$
levelC = intercept_{phaseC} - intercept_{phaseA}  - n_{phaseA+B} * slope_{phaseA} - 10 * slope_{phaseB}
$$ {#eq-contrast-fp-1}

$$
33.527 \approx 68.873 - 60.618 - 20 * -1.915 - 10 * 1.303 
$$
