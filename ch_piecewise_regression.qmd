...
This contrast answers the following question:

Level question: *Where would the value be if the slope of phase A (slope A = trend) continued through phase B until the beginning of phase C and what is the difference to the actual value at the first measurement of phase C?*

*Alternatively: What would the value be if there had been no level or slope effect in phase B, but only the continuing trend effect, and what is the difference to the actual value at the first measurement in phase C?*

Slope question: *What is the difference between the slope in phase C and the slope in phase A (slope A = trend)?*
...
```{r}
#| echo: false
#| label: fig-contrast-first-preceding
#| fig-cap: "LevelC-effect: The difference between the values where the blue arrow is pointing. SlopeC effect: The difference of the slopes of the two red regression lines."
exampleABC$Marie %>% 
  subset(phase != "B") %>%
  scplot() %>% 
  add_statline("trend", color = "darkred") %>%
  add_statline("trendA", color = "darkred") %>%
  set_yaxis(limits = c(0,100)) %>%
  add_arrow(x0 =21, y0 = 25,x1 = 21, y1 = 67,ends = "both", color = "darkblue",type = "closed") %>%
  add_text("Level C", x = 20, y = 50, hjust = 1) %>%
  add_theme("basic") %>% 
  set_casenames(" ")
```

The `levelC`-effect is the difference between the estimated first measurement of phase C (intercept C) and the extrapolated values of the trend line (slope A) at the first measurement of phase C.
...
Level question: *What would be the value if phase B had continued to the beginning of phase C and what is the difference to the actual value at the first measurement of phase C?*

Slope question: *What is the difference between the slope of phase C and the slope of phase B?*
...
Level question: What would the value be if phase A had continued to the beginning of phase C, assuming that a slope effect of phase B exists (but no level effect)? And what is the difference to the actual value at the first measurement of phase C?
...
Level question: What would the value be if phase B had continued to the beginning of phase C, assuming that phase B did not have a slope effect? And what is the difference to the actual value at the first measurement of phase C?

Slope question: *What is the difference between the slope of phase C and the slope of phase A (slope phase A = trend)?*
