...
The output provides the degrees of freedom and the deviance of the residuals.
Deviance is a measure of a model’s goodness of fit and is calculated as  $-2 \times \text{Likelihood}$ . The test compares the difference in degrees of freedom ($\Delta Df = 1$) and deviance ($\Delta \text{Deviance} = 1570.8$) between the two models.

Since both models assume Gaussian-distributed data, an F-test is performed. The F-value is computed as:

$$
F = \frac{\Delta \text{Deviance}}{\text{Dispersion}} = \frac{1570.8}{\frac{840.13}{18}} = \frac{1570.8}{46.67389} = 33.655
$$
...
This computes a $\chi^2$ test for $\Delta \text{Deviance} = 33.655$ with $\Delta Df = 1$, where the p-value is given by:

```{r}
pchisq(33.655, 1, lower.tail = FALSE)
```

We can also compare all four models as incremental nested models in a single step:

```{r}
anova(mod0, mod1, mod2, mod3)
```

Each model is compared to its predecessor. Since all models are nested and derived from the same data, the dispersion parameter for all comparisons is taken from the most complex model, mod3 (dispersion = 30.11).
