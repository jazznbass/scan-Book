...
$$
y_i = \beta_1 + \beta_2 (time_i-time_1) + \beta_3 phase_i + \beta_4 (time_i-time_{n_A+1})*phase_i 
$$

Where:

$i$ := a specific measurement.

$\beta_1$ := the intercept.

$\beta_2 time$ := the continuous effect of measurement time (the trend effect).

$\beta_3 phase$ := the discrete effect of a phase switch (the level effect).

$\beta_4 (time-n_{A+1})*phase$ := the continuous effect of a phase switch (the slope effect).

$n_{A+1}$ := the first measurement of phase B.

For $\beta_2$ and $\beta_4$, the time variables are shifted to start with a 0 at the first measurement of the respetive phase.
:::
...
In this example, the intercept is the score estimation at the beginning of the study (here: at the first measurement-time, see @fig-plm-ex-1). The trend effect (variable mt) is 0.100. That is, the score increased 0.100 with each measurement-time. As this example has a total of 20 measurements, the overall increase due to the trend is $19 * 0.100 = 1.9$ points. The level effect (variable phase with level B) is 7.858. That is, all measurements of phase B are increase by 7.858 points. The slope effect (for phase B) is 1.525. That is, with each measurement following the first measurement of phase B, the score increases by 1.525. As this example has 15 phase B measurements, the total increase due to the slope Effect is $14 * 1.525 = 21.35$ points.

Based on these values, each data-point can be estimated. For example, the last measurement ($i=20$) is estimated with $54.400 + 19 * 0.100 + 7.858 + 14 * 1.525 = 85.508$.
...
Since scan version `0.54.4` plm allows to set a contrast argument. `contrast = "first"` (the default) will compare all slope and level-effects to the values in the first phase. `contrast = "preceding"` will compare the slope and level-effects to the preceding phase.
