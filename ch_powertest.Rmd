...
The `design_rSC()` function sets up a single-case design. You can define various parameters of that design:
...
"phase.design", "A list defining the length and label of each phase. E.g., phase.length = list(A1 = 10, B1 = 10, A2 = 10, B2 = 10). Use vectors if you want to define different values for each case phase.length = list(A = c(10, 15), B = c(10, 15).",
...
print_table(tab, "Core arguments of the design_rSC function")
...
When conduction a power_test you firstly need to define a design which you like to be tested.
...
Then you have to choose the statistical method. The `power_test` function applies three methods by default: plm, randomization test, and TauU.
...
print_table(out, "Statistical methods", width_cols = c("10em", "7em", "50em"))

```

### Advanced methods

*Note: You need specific knowledge on how to create functions in R and on data structures to follow all aspects this section.*
...
Here is an example of a fast plm function for poisson distributed data based on the `fastglm` package:
