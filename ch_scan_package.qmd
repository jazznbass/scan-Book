...
For installing this version directly from within R, you can apply the `pak` function from the `pak` package[^ch_scan_package-1] with

[^ch_scan_package-1]:  Make sure you have installed the `pak` package before with `install.packages("pak")`.
