...
Now we introduce our prior knowledge: an Intercept of 50, trend and slope effect are 0, and a level effect of 10. Moreover, we assume that our previous knowledge is quite uncertain (i.e., a weakly informative prior). `mu` sets the prior values for the four paramters in the order as they apper in the regression model. `V` is a diagonalmatrix of the variances for these estimations. The V matrix sets the strength of the prior. Here, we set values of 100 which is a standard deviation (SD^2 = 10^2 = 100):
...
Now we assume, that we are somewhat certain (i.e., a prior of middle strength) by setting the variances to 10 (SD ~ 3.2):
...
Now we make somewhat wrong and uncertain assumptions:
...
And finally, we make the wrong assumptions with middle strong certainty:
