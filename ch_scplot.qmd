...
Plotting the data is a first important approach of analyzing. After you build an *scdf* the `scplot` command helps to visualize the data. When the `scdf` includes more than one case a multiple baseline figure is provided.

`scplot` is an add-on package to `scan` for visualizing single-case data. This set of functions replaces the `plot.scdf()` (short: `plot()`) function already included in `scan` (see @sec-plotsc). Here are some advantages of using `scplot` over the previous scan `plot` function:

-   `scplot` creates better scaled graphics.
...
For the time being, the "old" `plot.scdf` will be kept in future versions of `scan`.
...
You start by providing an scdf object (see @sec-scdf) to the `scplot()` function (e.g. `scplot(exampleAB)`). This already creates a default plot. Now you use a series of pipe-operators (`%>%` or `|>`) to add and change characteristics of the resulting plot. For example:
