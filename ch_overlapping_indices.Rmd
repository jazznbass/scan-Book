...
dat <- random_scdf(design(1, level = 1.0),round = 0, seed = 1212, random.names = "male")
plot(dat, style = "grid",lines = list("maxA", col = "red", lwd = 3))
```
...
dat <- random_scdf(design(1, level = 0.4,phase_design = list(A = 8, B = 12)), round = 0, seed = 1232, random_names = "male")
...
dat <- random_scdf(design(1, phase_design = list(A = 10, B = 15), level = 0.4),round = 0, seed = 9871, random_names = "female")
...
```{r}
dat <- scdf(c(2,0,1,4,3,5,9,7,8), phase_design = c(A = 4, B = 5))
res <- tau_u(dat)
